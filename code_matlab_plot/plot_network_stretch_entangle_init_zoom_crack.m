...
function plot_network_stretch_entangle_init_zoom_crack(x, x_init, neighbour_matrix, chain_num, chain_init_len, chain_info, is_cross_linker, fig_name)
...
%     near_crack_node_list = [];
%     interval_in_x = 3;
%     interval_in_y = 3;
%     for i = (round(num_width / 2) + 1 - interval_in_x) : (round(num_width / 2) + 1 + interval_in_x)
%     for j = (round(num_layer / 2) - interval_in_y) : (round(num_layer / 2) + 1 + interval_in_y)
%         node_index = local_to_global_index(i, j, num_width);
%         near_crack_node_list = [near_crack_node_list, node_index];
%     end
%     end
% 
%     plotted_chain = [-1, -1];
% 
%     for i = near_crack_node_list
% 
%         for j = 1:chain_num(i)
% 
%             % node index in this chain
