...
function plot_network_stretch_entangle_num_lay_wid(x, neighbour_matrix, chain_num, chain_init_len, chain_info, is_cross_linker, fig_name, num_layer, num_width)
...
stretch_min = 1.0;
stretch_max = 6.0;
%     for index_i = 1:(num_width-1)
%     for index_j = 1:num_layer        
%         i = (index_j - 1) * num_width + index_i;
%         for j = 1:chain_num(i)
%             stretch_min = min(stretch_min, strech_info(i,j));
%             stretch_max = max(stretch_max, strech_info(i,j));
%         end
...
if data_c >= stretch_max
    data_c = stretch_max;
                    ...
                    plot(data_x, data_y, 'Color', cmap(colorIndex, :), 'LineWidth', 2.5);
    ...
    for i = 1:dof
        if (is_cross_linker(i) == 1)
            plot(x(i,1), x(i,2), 'o', 'MarkerFaceColor', 'k', 'MarkerEdgeColor', 'k', 'MarkerSize', 13, 'linewidth', 1.2);
            % plot(x(i,1), x(i,2), 'b.');
        else
            plot(x(i,1), x(i,2), 'x', 'MarkerFaceColor', 'r', 'MarkerEdgeColor', 'r', 'MarkerSize', 17, 'LineWidth', 1.6);
            % plot(x(i,1), x(i,2), 'r.');
        end
    end
